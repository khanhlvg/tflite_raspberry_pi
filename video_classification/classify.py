...
def run(model: str, label: str, max_results: int, num_threads: int,
    ...
    while cap.isOpened():
        ...
        # Ensure that frames are feed to the model at {_MODEL_FPS} frame per seconds as required in the model specs.
        ...
        # Notes: Frames that aren't fed to the model are displayed to make the video look smooth.
        # Show the FPS .
        ...
        time_per_infer_text = 'Time per inference: {0}ms'.format(int(time_per_infer * 1000))
        text_location = (_LEFT_MARGIN, _ROW_SIZE * 2)
        cv2.putText(image, time_per_infer_text, text_location, cv2.FONT_HERSHEY_PLAIN,
                    _FONT_SIZE, _TEXT_COLOR, _FONT_THICKNESS)
        ...
        for idx, category in enumerate(categories):
            ...
            # Skip the first 2 lines that are used to show fps and time per inference.
