...
class VideoClassifierTest(unittest.TestCase):
  ...
  def _run_classification_with_frames(self, classifier: VideoClassifier, frames: List[np.ndarray]) -> [Category]:
  ...
  def test_default_option(self):
    ...
    self.assertEqual(_GROUND_TRUTH_LABEL, top_1_category.label,
                     'Label {0} does not match with ground truth {1}'.format(top_1_category.label, _GROUND_TRUTH_LABEL))
    self.assertLessEqual(_GROUND_TRUTH_MIN_SCORE, top_1_category.score,
                         'Classification score {0} is smaller than threshold {1}'.format(top_1_category.score,
                                                                                         _GROUND_TRUTH_MIN_SCORE))
