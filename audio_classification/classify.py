...
def run(model: str, max_results: int, score_threshold: float, num_threads: int, enable_edgetpu: bool) -> None:
  ...
  # We'll try to run inference every interval_between_inference seconds. This is usually half of the model's input
  # length to create an overlapping between incoming audio segments to improve classification accuracy.
  overlapping_factor = 0.5
  input_length_in_second = float(len(tensor_audio.buffer)) / tensor_audio.format.sample_rate
  interval_between_inference = input_length_in_second * overlapping_factor
  ...
  # Loop until the user close the result plot.
  while True:
    # Wait until at least interval_between_inference has passed since the last inference.
    now = time.time()
...
def main():
  ...
  run(args.model, int(args.maxResults), args.scoreThreshold, int(args.numThreads), bool(args.enableEdgeTPU))
