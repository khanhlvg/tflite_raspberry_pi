...
class TensorAudio(object):
  ...
  def load_from_audio_record(self, audio_record: AudioRecord) -> None:
    ...
    if not len(data):
      # Skip if the audio record's data is empty.
      return
  ...
  def load_from_array(self, src: np.ndarray):
    ...
    # Shift the internal buffer backward and add the incoming data to the end of the buffer.
    shift = len(src)
...
class AudioClassifier(object):
  ...
  def __init__(self,
               model_path: str,
               options: AudioClassifierOptions = AudioClassifierOptions()) -> None:
    """Initialize the Audio classification model.

    Args:
        model_path: Path of the TFLite Audio classification model.
        options: The config to initialize an audio classifier. (Optional)
    ...
    self._audio_format = AudioFormat(input_content_props["channels"], input_content_props["sample_rate"])
  ...
  def create_input_tensor_audio(self) -> TensorAudio:
    ...
    """
    return TensorAudio(audio_format=self._audio_format,
                       sample_count=self._input_sample_count)
  ...
  def classify(self, tensor: TensorAudio) -> List[Category]:
    ...
    self._interpreter.resize_tensor_input(self._waveform_input_index, [len(input_tensor)], strict=True)
    self._interpreter.allocate_tensors()
    self._interpreter.set_tensor(self._waveform_input_index, input_tensor.astype(np.float32))
  ...
  def _postprocess(self, output_tensor: np.ndarray) -> List[Category]:
    """Post-process the output tensor into a list prediction.
    ...
    categories = [Category(label=self._labels_list[idx], score=scores[idx])
